...
const BASE_URL = "https://www.acelin.com.br";

class AcicLindoOesteScraper {
  constructor() {
  ...
  async coletarDetalhesEmpresa(linkDetalhes) {
    try {
      ...
      // Busca email por ícone de envelope
      $(".dados-associado").each((_, elemento) => {
        const $elemento = $(elemento);
...
async function runAcicLindoOeste() {
  const scraper = new AcicLindoOesteScraper();
  return await scraper.iniciarScraping();
}

module.exports = runAcicLindoOeste;
